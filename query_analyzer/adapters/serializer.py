...
class ReportSerializer:
    ...
    def to_json(report: QueryAnalysisReport, indent: int = 2) -> str:
        ...
        - Métricas y datos crudos del motor
    ...
    def to_markdown(report: QueryAnalysisReport) -> str:
        ...
        if report.plan_summary:
            ...
            lines.append(report.plan_summary)
            lines.append("")
        ...
        if report.ai_analysis:
            lines.append("## Analisis de IA")
            lines.append("")
            lines.append(
                "> Esta seccion es generada por IA y no corresponde a datos crudos del motor."
            )
            lines.append("")
            lines.append(report.ai_analysis.summary)
            if report.ai_analysis.observations:
                lines.append("")
                lines.append("### Observaciones")
                lines.extend(f"- {item}" for item in report.ai_analysis.observations)
            if report.ai_analysis.recommendations:
                lines.append("")
                lines.append("### Recomendaciones")
                lines.extend(f"- {item}" for item in report.ai_analysis.recommendations)
            if report.ai_analysis.suggested_query:
                lines.append("")
                lines.append("### Consulta sugerida")
                lines.append("```sql")
                lines.append(report.ai_analysis.suggested_query)
                lines.append("```")
            lines.append("")
